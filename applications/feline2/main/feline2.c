...
#include	<signal.h>
#include	<rpc/rpc.h>
#include	"/home/warner/staff/mikew/made1.0//masc/rpcs/rem_inst.h"
#include	"/home/warner/staff/mikew/made1.0//include/paths.h"

	/* default values */

#define	DEFAULT_DFLAG	1
#define	DEFAULT_READER	1
#define	DEFAULT_SUNVIEW	1
#define	DEFAULT_AWT	1
#define	DEFAULT_DEBUGGER	0
...
void	install_locally()
{
	install_dkbs_locally();
	sleep(20);
	install_debugger_locally();
	install_reader_locally();
	install_wintool_locally();
	...
	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"feline2 : initiating blood.\n");
		execlp(EXEC_POP,"pop11", "blood", (char *)0);
		fprintf(stderr,"blood stopped\n");
		exit(1);
	}

	sleep(25);

	/* now the agent haemorrhagic */

	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"feline2 : initiating haemorrhagic.\n");
		execlp(EXEC_POP,"pop11", "haemorrhagic", (char *)0);
		fprintf(stderr,"haemorrhagic stopped\n");
		exit(1);
	}

	sleep(25);

	/* now the agent aplasia */

	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"feline2 : initiating aplasia.\n");
		execlp(EXEC_POP,"pop11", "aplasia", (char *)0);
		fprintf(stderr,"aplasia stopped\n");
		exit(1);
	}

	sleep(25);

	/* now the agent haemolytic */

	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"feline2 : initiating haemolytic.\n");
		execlp(EXEC_POP,"pop11", "haemolytic", (char *)0);
		fprintf(stderr,"haemolytic stopped\n");
		exit(1);
	}

	sleep(25);

	/* now the agent initial */

	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"feline2 : initiating initial.\n");
		execlp(EXEC_PWM, "pwmtool", "-Wp" , "0", "0", "-Wh" , "25" , "-Ww", "80" ,"pop11", "initial", (char *)0);
		fprintf(stderr,"initial stopped\n");
		exit(1);
	}
...
void	install_called2()
{


	/* now the agent blood */

	if(strcmp(hostname,"winograd") == 0) /* then install this agent */
	{
		pid[pcount] = fork();
		if(pid[pcount++] == 0)	/* then child */
		{
	...
	if(strcmp(hostname,"schank") == 0) /* then install this agent */
	{
		pid[pcount] = fork();
		if(pid[pcount++] == 0)	/* then child */
		{
