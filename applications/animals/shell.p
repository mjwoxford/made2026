 ...
 *  We start with some general functions
 */
...
define  one_step_forward -> retval;
    ...
    define  one_step_forward_aux -> retval;
            ...
            for fcrule in rulebase do
                ...
                then
            fcrule(1) -> nm;
                    shell_trace([ '[Trace] : Firing forward chaining rule' ^nm]);
                        fire(nm);
                    assert(nm, true);
                        true -> retval;
                endif;
...
define  ask(name) -> retval;
    ...
    vars    resp flag dummy ;

    /* if agent window is open then update current action */

	'Sleeping' -> current_status;
	'Asking ' >< name -> current_action;
	update_status();

	/* now get on with the meat */
	...
	/* now re-update the status */

	'Active' -> current_status;
	'Backward chaining' -> current_action;
	update_status();
...
define  evaluate(name) -> retval;
	...
	elseif operation_mode = DISTRIBUTED then
		if remote(name) then

    			/* update current action */
	
			'Requesting ' >< name -> current_action;
			'Sleeping' -> current_status;
			'' >< name -> current_goal;
			update_status();

			get_agent_who_knows(name) -> ag;
			pop11_request(ag,name) -> retval;
...
define  meta_chain();
    ...
    define  meta_chain_aux();
            vars mrule nm cond code;

        shell_trace([ '[Trace] : Calling meta rule chainer' ]);
