...
void	install_locally()
{
	...
	{
		fprintf(stderr,"animals : initiating birds.\n");
		execlp(EXEC_POP,"pop11", "birds", (char *)0);
		fprintf(stderr,"birds stopped\n");
		exit(1);
	}

	sleep(25);

	/* now the agent carnivores */

	pid[pcount] = fork();
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"animals : initiating carnivores.\n");
		execlp(EXEC_POP,"pop11", "carnivores", (char *)0);
		fprintf(stderr,"carnivores stopped\n");
		exit(1);
	}
	...
	if(pid[pcount++] == 0)	/* then child */
	{
		fprintf(stderr,"animals : initiating user.\n");
		execlp(EXEC_POP,"pop11", "user", (char *)0);
		fprintf(stderr,"user stopped\n");
		exit(1);
	}
...
void	install_called2()
{
	...
	if(strcmp(hostname,"turing") == 0) /* then install this agent */
	{
		pid[pcount] = fork();
		if(pid[pcount++] == 0)	/* then child */
		{
	...
	if(strcmp(hostname,"papert") == 0) /* then install this agent */
	{
		pid[pcount] = fork();
		if(pid[pcount++] == 0)	/* then child */
		{
